...
const displayIsuueDetails = (issue) => {
  ...
  detailBox.innerHTML = ` <h2 class="font -bold text-2xl text-black">${issue.title}</h2>
<div class="flex gap-5  ">
  <div class="badge  ${issue.status === 'open' ?
  'badge-success' : 'badge-primary'  } ">${issue.status}
</div>
<div>
...
<div class="flex gap-2 flex-wrap mb-4  py-7">
    ...
    <p class="text-gray-500 pb-8">${issue.description}</p>
    <div class="flex justify-around">
      <p class="text-gray-500">Assignee: <br><span class="font-extrabold text-xl text-black">${issue.author}</span></p>
      <p class="text-gray-500">Priority <br><span class="text-xs font-bold px-3 py-1 rounded-full border  ${issue.priority === 'high' ? 'border-red-300 text-red-500 bg-red-100'
